...
const Feedback = {
  addNewFeedbackOnOrgByUser: function(req, res) {
    ...
    if (
      params["org"] &&
      params["overalEstimation"] &&
      params["difficultingProc"] &&
      params["speed"] &&
      params["polite"]
    ) {
      try {
        db.any(
          'INSERT INTO public.feedback (user_id, org_id, comment, "overalEstimation", "difficultingProc", speed, polite, "hasRecommended", user_name, email) ' +
  ...
  addNewResponseOnFeedback: function(req, res) {
    ...
    if (params["user"] && params["response"] && params["feedback_id"]) {
      try {
        db.any(
          "INSERT INTO public.response_on_feedback (user_id, response, feedback_id) " +
            "VALUES ($1, $2, $3)",
          [params["user"], params["response"], params["feedback_id"]]
        )
          .then(function(data) {
            db.any(
              'UPDATE public.feedback set "hasResponse" = true where id = $1',
              params["feedback_id"]
            )
              .then(function(data) {
                res.send(data);
